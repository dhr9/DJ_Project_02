...
##arduino = startup('/dev/tty.usbmodem14141')
...
def wait_for_reply() :
    ...
    while(arduino.inWaiting() == 0) :
        i = 0
    if(arduino.read(arduino.inWaiting()) == 's') :
        print 'okay sirji :)'
        return True
    else :
        print 'error !!!  ;('
...
def write(id_ ,position_low,position_high,speed_low,speed_high) :
    ...
    _not_checksum_ = not_checksum(checksum) 
    data = '\xff\xff' + chr(id_) + '\x07\x03\x1e' + chr(position_low)+ chr(position_high) + chr(speed_low) + chr(speed_high) + chr(_not_checksum_)
    rw('w')
    ...
    if(reply == True) :
        ...
        print('breaking out, breaking bad :( ')
    rw('r')
    reply = wait_for_reply()
    if(reply == True) :
        time.sleep(0.005);
...
def read(id_,address,length) :
    checksum = [id_ , 0x04 , 0x02 , address,length]
    __not_checksum__ = not_checksum(checksum)
    data = '\xff\xff' + chr(id_) + '\x04\x02'+ chr(address) + chr(length) + chr(__not_checksum__)
    rw('w')
    reply = wait_for_reply()
    if(reply == True) :
        time.sleep(0.005)
...
def rs232_check() : #connect the tx and rx of the rs232 module to check it
    dynamixel.write('hi there ')
    time.sleep(0.1)
    data = dynamixel.read(dynamixel.inWaiting())
    print(data)
...
def hope_that_this_works() :
    rw('w')
    time.sleep(0.1)
    dynamixel.write('\xff\xff\x01\x04\x03\x09\x00\xee')
    time.sleep(0.1)
    check()
