...
the instruction packet for the dynamixel consists of 6 parts :
    ...
    part2) id of the motor :        you need to specify the id of the motor in case that more than one motors are
...
def dhr1(angle):
    dhr_write(0x01,angle,0xff,0x03)
    time.sleep(1)
    #dhr_read_print()

def dhr2(angle):
    dhr_write(0x02,angle,0xff,0x03)
    time.sleep(1)
    #dhr_read_print()
...
def dhr_ping(id_):
    checksum = [id_ , 0x01 , 0x02 , 0x01]
    __not_checksum__ = not_checksum(checksum)
    data = '\xff\xff' + chr(id_) + '\x01\x02\x01' + chr(__not_checksum__)
    #print list(data)
    rw('w')
    time.sleep(0.001)
    dynamixel.write(data)
